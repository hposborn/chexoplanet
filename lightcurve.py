...
class lc():
    ...
    def plot(self, plot_rows=1, timeseries=['flux'], jump_thresh=10, ylim=None, xlim=None, bin_only=False,
        ...
        if (savepng or savepdf) and not hasattr(self,'savefileloc'):
            self.savefileloc = os.path.join(tools.chexo_tablepath,tools.id_dic[self.mission]+str(id).zfill(11),tools.id_dic[self.mission]+str(id).zfill(11)+'.pkl.gz') if savefileloc is None else savefileloc
        if savepng:
            fig.savefig(self.savefileloc.replace('.pkl.gz','_lc.png'))
        if savepdf:
            fig.savefig(self.savefileloc.replace('.pkl.gz','_lc.pdf'))
...
class multilc(lc):
    ...
    def __init__(self,id,mission,radec=None, load=True, do_search=True,flx_system='ppt',
        ...
        Args:
            ...
            savefileloc (str,optional): File location to load and/or save lightcurve. Defaults to a new target-specific folder in `tools.chexo_tablepath`
        ...
        """
        self.savefileloc = os.path.join(tools.chexo_savepath,tools.id_dic[mission]+str(id).zfill(11),tools.id_dic[mission]+str(id).zfill(11)+'.pkl.gz') if savefileloc is None else savefileloc
    ...
    def get_tess_sectors(self,id=None,sectors='all', **kwargs):
        ...
        epoch=pd.read_csv(tools.chexo_tablepath+"/tess_lc_locations.csv",index_col=0)
    ...
    def plot(self, plot_rows=None, timeseries=['flux'], ylim=None, xlim=None, overwrite=False,norm_all_timeseries=True,bin_only=False,
        ...
        plt.tight_layout()
        if savepng:
            plt.savefig(self.savefileloc.replace('.pkl.gz','_lc.png'))
        if savepdf:
            plt.savefig(self.savefileloc.replace('.pkl.gz','_lc.pdf'))
